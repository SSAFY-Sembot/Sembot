...
export const fetchMembersByPage = createAsyncThunk("members/fetchMembersByPage", async (params: FetchMembersParams) => {
  const searchCondition = params.searchValue ? { [params.searchType || "name"]: params.searchValue } : undefined;
...
const memberSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMembersByPage.fulfilled, (state, action: PayloadAction<Member>) => {
        // 요청 성공 시 상태를 업데이트
