...
const Chat: React.FC = () => {
	...
	const newChatProp: ButtonWithIconProps = {
		btnName: "새채팅",
		styleName:
			"flex bg-transparent border border-white text-white py-2 px-4 rounded mx-1 hover:bg-blue-900 transition-colors duration-200 ease-in-out",
	...
	const handleGenerateResponse = async (message: string) => {
		try {
			...
			// TODO : 채팅 저장하기
			const chatId = qnas[qnas.length - 1]
				? qnas[qnas.length - 1].chatId + 1
				: 1;
